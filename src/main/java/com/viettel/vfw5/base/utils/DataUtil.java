...
//
    ...
    public static Map<String, OrdersDTO> putOrderToMap(List<OrdersDTO> lstGoods) {
        Map<String, OrdersDTO> mapGoodsDTO = new HashMap<>();
        if (lstGoods != null) {
            for (OrdersDTO goodsDTO : lstGoods) {
                mapGoodsDTO.put(goodsDTO.getOrderId(), goodsDTO);
            }
        }
        return mapGoodsDTO;
    }

    public static Map<String, GoodsDTO> putGoodsCodeToMap(List<GoodsDTO> lstGoods) {
        Map<String, GoodsDTO> mapGoodsDTO = new HashMap<>();
        if (lstGoods != null) {
            for (GoodsDTO goodsDTO : lstGoods) {
    ...
    public static List<String> buildSingleSerial(List<StockTransSerialDTO> lstSerial,
            Map<StockTransSerialDTO, List<String>> mapStockTransSerial2ListSerial) {
        List<String> lstSerial2Search = new ArrayList<>();
        StringBuilder lstSingleSerial = new StringBuilder();
        String fromSerial;
        String toSerial;
        int count = 0;
        List<String> listSerialEachStockTransSerial;
        for (StockTransSerialDTO s : lstSerial) {
            listSerialEachStockTransSerial = new ArrayList<>();
            fromSerial = s.getFromSerial();
            toSerial = s.getToSerial();
            //Lay danh sach nhung serial don le khong import len theo dai
            if (fromSerial.equals(toSerial)) {
                lstSingleSerial.append(Constants.COMMA);
                lstSingleSerial.append(s.getFromSerial());
                count++;
                if (count > 900) {
                    lstSerial2Search.add(lstSingleSerial.substring(1));
                    count = 0;
                    lstSingleSerial = new StringBuilder();
                }
                //Them vao map
                listSerialEachStockTransSerial.add(toSerial);
                mapStockTransSerial2ListSerial.put(s, listSerialEachStockTransSerial);
            } else {
                //Neu khong thi tach thanh danh sach cac serial don le
                String prefixSerial;
                ...
                if (!StringUtils.isInteger(fromSerial) || !StringUtils.isInteger(toSerial)) {
                    lstSingleSerial.append(Constants.COMMA);
                    lstSingleSerial.append(fromSerial);
                    count++;
                    if (count > 900) {
                        lstSerial2Search.add(lstSingleSerial.substring(1));
                        count = 0;
                    ...
                    for (Long lSerial = Long.parseLong(suffixFromSerial); lSerial <= Long.parseLong(suffixToSerial); lSerial++) {
                        ...
                        tmpSerial = prefixSerial + tmpSuffixSerial;
                        lstSingleSerial.append(Constants.COMMA);
                        lstSingleSerial.append(tmpSerial);
                        count++;
                        if (count > 900) {
                            lstSerial2Search.add(lstSingleSerial.substring(1));
                            count = 0;
        ...
        if (!lstSingleSerial.toString().isEmpty()) {
            lstSerial2Search.add(lstSingleSerial.substring(1));
        }
        return lstSerial2Search;
    }
}
